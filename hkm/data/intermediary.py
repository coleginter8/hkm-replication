...
def build_capital_ratio(
    ...
    for t in date_range:
        ...
        for gvkey in active_gvkeys:
            ...
            # Use datadate (fiscal quarter-end) as the alignment date.
            # This is the pre-rdq approach: a filing with datadate <= t is
            # considered available at t. While this can include filings not
            # yet publicly reported, the simpler alignment avoids the
            # look-ahead artefacts introduced by the rdq-based availability
            # date (which can misalign quarterly book capital by 1-3 months
            # relative to CRSP month-end market equity, flipping cyclical
            # sign for GDP, Unemployment, and Market volatility correlations).
            comp_sub = comp_df[comp_df["gvkey"] == gvkey].copy()
            if comp_sub.empty:
                continue
            comp_sub = comp_sub[comp_sub["datadate"] <= t_timestamp]
            if comp_sub.empty:
                continue
